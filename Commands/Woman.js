...
if (!args[1]) return message.channel.send(`Doğru Kullanım \`${client.nufrain.Prefix}woman @Kullanıcı (İsim) (Yaş)\``)
if (!args[2]) return message.channel.send(`Doğru Kullanım \`${client.nufrain.Prefix}woman @Kullanıcı (İsim) (Yaş)\``)
...
if(!Kullanıcı) return message.channel.send(`Doğru Kullanım \`${client.nufrain.Prefix}woman @Kullanıcı (isim) (yaş)\``).then(x => x.delete({timeout: 3000}));
if(!Name) return message.channel.send(`Doğru Kullanım \`${client.nufrain.Prefix}woman @Kullanıcı (isim) (yaş)\``).then(x => x.delete({timeout: 3000}));
if(!Age) return message.channel.send(`Doğru Kullanım \`${client.nufrain.Prefix}woman @Kullanıcı (isim) (yaş)\``).then(x => x.delete({timeout: 3000}));
...
Kullanıcı.roles.cache.has(client.nufrain.BoosterRole) ? Kullanıcı.roles.set(client.nufrain.Woman.concat(client.nufrain.BoosterRole)) : Kullanıcı.roles.set([client.nufrain.Woman]);  
...
message.channel.send(new MessageEmbed().setColor("RANDOM").setAuthor(message.author.tag, message.author.avatarURL({dynamic: true})).setDescription(`${Kullanıcı} (\`${Kullanıcı.id}\`) başarılı olarak <@&${client.nufrain.Woman}> olarak kaydedildi.
        ${messagess}`)
...
exports.conf = {
  name: "woman @Kullanıcı (isim) (yaş)",
  aliases: ["woman", "Woman", "WOMAN", "Kız", "kız", "KIZ", "kiz", "Kiz", "KİZ", "kadın", "Kadın", "KADIN", "kadin", "Kadin", "KADİN", "k", "K"],
  guildOnly: true
};
